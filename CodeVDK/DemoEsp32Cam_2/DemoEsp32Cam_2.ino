...
const char* uploadUrl = "http://192.168.132.148:8000/predict";  
const char* statusUrl = "http://192.168.132.148:8000/check_status";  
const char* logServerUrl = "http://192.168.132.148:8000/log";  
...
void sendFiveImages() {
    ...
    for (int i = 1; i <= 5; i++) {
        sendLog("📷 Gửi ảnh " + String(i) + " trên 5...");
        sendImage();
...
void loop() {
    ...
    if (currentState && !isDetecting) { // Bắt đầu lần nhận diện 5 ảnh mới
        detectionCount++;
        isDetecting = true;
        sendLog("📷 Phát hiện vật cản, bắt đầu lần nhận diện 5 ảnh...");
        
        sendFiveImages(); // Gửi ảnh bất kể trạng thái server
        sendLog("✅ Hoàn thành gửi 5 ảnh. Chờ server xử lý...");
